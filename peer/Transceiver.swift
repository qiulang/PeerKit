...
public class Transceiver: SessionDelegate {
    ...
    public func finishReceivingResource(_ name: String, fromPeer peerID: MCPeerID, atURL localURL: URL) {
    ...
    public func startReceivingResource(_ name: String, session: MCSession, fromPeer peerID: MCPeerID, progress: Progress) {
        didStartReceivingResource(name, session: session, fromPeer: peerID, progress: progress)
        print("startReceivingResourceWithName: \(progress.totalUnitCount) : \(progress.completedUnitCount)");
    }
}
